"""
actions/apply_to_jobs/apply_to_craigslist.py

Open a Craigslist job listing in a browser and apply to it.
...
def apply(job: dict[str, Any]) -> None:
    url = job.get("url")
    if not url:
        raise ValueError(f"craigslist job missing url: {job}")
