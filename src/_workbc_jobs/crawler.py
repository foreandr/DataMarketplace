...
try:
    ...
    from _workbc_jobs.schema import SCHEMA
...
class WorkbcJobsCrawler:
    def __init__(self, name: str = "_workbc_jobs"):
        self.name        = name
...
def dedup_database(db_path: Path | None = None) -> int:
    ...
    if db_path is None:
        db_path = Path(__file__).resolve().parents[2] / "src" / "_workbc_jobs" / "database.sqlite"
    if not db_path.exists():
