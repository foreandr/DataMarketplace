...
def _write_crawler_stub(module_name: str, class_name: str, jsonify_class_name: str) -> None:
    path = _crawler_dir(module_name) / "crawler.py"
    if path.exists():
        raise FileExistsError(f"Already exists: {path}")
    # Raw string so \033 and {expr} pass through untouched into the output file.
    # Tokens: %%MODULE%% %%CLASS%% %%JSONIFY%% — replaced below.
    template = r'''"""Crawler for %%MODULE%%."""
from __future__ import annotations
...
try:
    from %%MODULE%%.jsonify import %%JSONIFY%%
...
def dedup_database(db_path: Path | None = None) -> int:
    ...
    if db_path is None:
        db_path = Path(__file__).resolve().parents[2] / "src" / "%%MODULE%%" / "database.sqlite"
    if not db_path.exists():
...
'''
    content = (
        template
        .replace("%%JSONIFY%%", jsonify_class_name)
...
def main(
    ...
    _write_crawler_stub(module_name, class_name, jsonify_class_name)
    print("  OK")
