...
try:
    from _saskjobs.jsonify import SaskjobsJsonify
    from _saskjobs.schema import SCHEMA
...
class SaskjobsCrawler:
    def __init__(self, name: str = "_saskjobs"):
        self.name        = name
    ...
    def run(self) -> None:
        ...
        base_url = "https://www.saskjobs.ca/jsp/joborder/listing.jsp?filter_by=2&catsort=0&page=@PAGE_NO"
        ...
        while True:
            for page_no in range(1, 176):
                try:
                    ...
                    jsonifier = SaskjobsJsonify(self.name)
...
def dedup_database(db_path: Path | None = None) -> int:
    ...
    if db_path is None:
        db_path = Path(__file__).resolve().parents[2] / "src" / "_saskjobs" / "database.sqlite"
    if not db_path.exists():
