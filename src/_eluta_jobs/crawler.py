...
try:
    from _eluta_jobs.jsonify import ElutaJobsJsonify
    from _eluta_jobs.schema import SCHEMA
...
class ElutaJobsCrawler:
    def __init__(self, name: str = "_eluta_jobs"):
        self.name        = name
    ...
    def run(self) -> None:
        ...
        for i, item in enumerate(items, 1):
            try:
                ...
                jsonifier  = ElutaJobsJsonify(self.name)
...
def dedup_database(db_path: Path | None = None) -> int:
    ...
    if db_path is None:
        db_path = Path(__file__).resolve().parents[2] / "src" / "_eluta_jobs" / "database.sqlite"
    if not db_path.exists():
