...
try:
    from _goodwork_jobs.jsonify import GoodworkJobsJsonify
    from _goodwork_jobs.schema import SCHEMA
...
class GoodworkJobsCrawler:
    def __init__(self, name: str = "_goodwork_jobs"):
        self.name        = name
    ...
    def run(self) -> None:
        ...
        base_url = "https://www.goodwork.ca/jobs/@PAGE_NO"
        ...
        while True:
            for page_no in range(1, 11):
                try:
                    ...
                    jsonifier = GoodworkJobsJsonify(self.name)
                ...
                print(f"[page {page_no}/10] {self.name} | rows={db_total}")
                self._maybe_push()

            print("HIT END, WAITING...")
            time.sleep(2 * 60 * 60)
...
def dedup_database(db_path: Path | None = None) -> int:
    ...
    if db_path is None:
        db_path = Path(__file__).resolve().parents[2] / "src" / "_goodwork_jobs" / "database.sqlite"
    if not db_path.exists():
