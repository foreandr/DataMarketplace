...
try:
    from _upwork_jobs.jsonify import UpworkJobsJsonify
    from _upwork_jobs.schema import SCHEMA
...
class UpworkJobsCrawler:
    def __init__(self, name: str = "_upwork_jobs"):
        self.name        = name
    ...
    def run(self) -> None:
        ...
        for i, item in enumerate(items, 1):
            try:
                ...
                jsonifier  = UpworkJobsJsonify(self.name)
...
def dedup_database(db_path: Path | None = None) -> int:
    ...
    if db_path is None:
        db_path = Path(__file__).resolve().parents[2] / "src" / "_upwork_jobs" / "database.sqlite"
    if not db_path.exists():
