"""Crawler for _charityvillage_jobs."""
...
try:
    from _charityvillage_jobs.jsonify import CharityvillageJobsJsonify
    from _charityvillage_jobs.schema import SCHEMA
...
class CharityvillageJobsCrawler:
    def __init__(self, name: str = "_charityvillage_jobs"):
        self.name        = name
...
def dedup_database(db_path: Path | None = None) -> int:
    ...
    if db_path is None:
        db_path = Path(__file__).resolve().parents[2] / "src" / "_charityvillage_jobs" / "database.sqlite"
    if not db_path.exists():
